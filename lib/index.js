...
function populateFakeParentLinks (parentIssues, childIssues) {
  childIssues.forEach((childIssue) => {
    const parentKey = _.get(childIssue, 'parentKey.data.key')
    const parentIssue = parentIssues[parentKey]
    if (parentIssue) {
      parentIssue.links.push({
        direction: 'inward',
        dstKey: childIssue.key,
        dstStatus: childIssue.status,
        dstStatusCategory: childIssue.statusCategory,
        label: 'is delivered by',
        srcKey: parentIssue.key,
        type: 'Parent'
      })
      childIssue.links.push({
        direction: 'outward',
        dstKey: parentIssue.key,
        dstStatus: parentIssue.status,
        dstStatusCategory: parentIssue.statusCategory,
        label: 'delivers',
        srcKey: childIssue.key,
        type: 'Parent'
      })
...
function processNewIssues (issuesByKey, newIssues) {
  ...
  newIssues.forEach((newIssue) => {
    ...
    const srcTerminal = this.options.terminalStatusCategories.includes(newIssue.statusCategory) ||
        this.options.terminalStatuses.includes(newIssue.status)

    // filter out links if both nodes are terminal
    newIssue.links = newIssue.links.filter((link) => {
      const dstTerminal = this.options.terminalStatusCategories.includes(link.dstStatusCategory) ||
          this.options.terminalStatuses.includes(link.dstStatus)
      return !srcTerminal || !dstTerminal
    })

    // filter out issues if they only have outward links and are terminal
    if (_.every(newIssue.links, {direction: 'outward'}) && srcTerminal) {
      logger.trace(`${newIssue.key} - terminal state with only outward links - skipping`)
      return
    }
    ...
    if (this.options.epicIssueTypes.includes(newIssue.type)) {
      epicIssues[newIssue.key] = newIssue
      logger.trace(`${newIssue.key} - is an epic`)
    }

    const parentKey = _.get(newIssue, 'parentKey.data.key')
    if (parentKey && !issuesByKey[parentKey]) {
      unvisitedIssues[parentKey] = parentKey
      logger.trace(`${newIssue.key} - ${newIssue.parentKey} parent not yet fetched`)
    }

    if (this.options.parentIssueTypes.includes(newIssue.type)) {
      parentIssues[newIssue.key] = newIssue
      logger.trace(`${newIssue.key} - is an parent`)
    }
  })
  const unvisitedIssueKeys = _.sortBy(Object.keys(unvisitedIssues), helpers.sortableKey)
  return {unvisitedIssueKeys, epicIssues, parentIssues}
}

async function fetchNewIssues (unvisitedIssueKeys, epicIssues, parentIssues) {
  let newIssues = []
  ...
  if (unseenEpicIssueKeys.length > 0 && this.options.epicKeyCustomField) {
  ...
  const unseenParentIssueKeys = Object.keys(parentIssues)
  if (unseenParentIssueKeys.length > 0 && this.options.parentKeyCustomField) {
    logger.info(`fetching unseen parent child issues`)
    const childIssues = await this.jiraClient.getIssuesByParentKey(unseenParentIssueKeys)
    logger.trace(`found ${Object.keys(childIssues).length} parent child issues`)
    newIssues = newIssues.concat(childIssues)
    populateFakeParentLinks(parentIssues, childIssues)
  }
 ...
 * @param {string} options.epicKeyCustomField - The epic key custom field
 * @param {string[]} options.epicIssueTypes - An array of epic issue types
 * @param {string} options.parentKeyCustomField - The parent key custom field
 * @param {string[]} options.parentIssueTypes - An array of parent issue types
...
class Jirascope {
  ...
  async fetch () {
    ...
    let newIssues = await this.jiraClient.getIssuesByQuery(this.options.query, 100)
    newIssues.forEach((newIssue) => { issuesByKey[newIssue.key] = newIssue }) // initial issues we always keep

    while (newIssues.length > 0) {
      const { unvisitedIssueKeys, epicIssues, parentIssues } = processNewIssues.call(this, issuesByKey, newIssues)
      newIssues = await fetchNewIssues.call(this, unvisitedIssueKeys, epicIssues, parentIssues)
    }
