  ...
  while (Object.keys(issues).length !== 0) {
    ...
    if (stack.length !== 0) {
      ...
      while (stack.length !== 0) {
        ...
        if (issue) {
          const node = _.omit(issue, ['links', 'analysis'])
          node.analysis = _.clone(issue.analysis)
 ...
 * @param {string[]} options.graphIssueTypes - An array of types that must be present in a graph
