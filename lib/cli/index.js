...
async function run (appName) {
  const defaultConfig = {
    ...
    terminalStatuses: ['Done', 'Reject'],
    terminalStatusCategories: ['Done'],
  ...
  // allowing for password and token to be used
  if (config.token) {
  ...
  const jirascope = new Jirascope(jiraClient, dataStore, {
    ...
    terminalStatusCategories: config.terminalStatusCategories,
