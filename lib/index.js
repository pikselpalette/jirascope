...
class Jirascope {
  ...
  async fetch () {
    ...
    while (newIssues.length > 0) {
      ...
      if (Object.keys(epicIssues).length > 0) {
        logger.info(`fetching epic child issues`)
        newIssues = newIssues.concat(await this.jiraClient.getIssuesByQuery(
          `"Epic Link" in (${Object.keys(epicIssues).join(', ')})`, 1000))
      }
    }
    this.issues = issues
  }
