...
function score (graph) {
  const edges = _.groupBy(graph.edges, 'srcKey')
  const nodes = _.keyBy(graph.nodes, 'key')
  const descendents = {}
  ...
  const size = (key) => {
    ...
    if (!node.analysis.leaf) {
      edges[key].forEach((edge) => {
        size(edge.dstKey)
        descendents[key] = descendents[key].concat(edge.dstKey, descendents[edge.dstKey])
      })
      descendents[key] = _.uniq(descendents[key])
    }
    node.analysis.size = descendents[key].length + 1
  }

  _.filter(graph.nodes, 'analysis.root').forEach((root) => {
    size(root.key)
  })
}
