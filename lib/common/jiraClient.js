...
class JiraClient {
  ...
  convertIssue (rawIssue) {
    ...
    if (this.options.epicKeyCustomField && rawIssue.fields[this.options.epicKeyCustomField]) {
      issue.epicKey = rawIssue.fields[this.options.epicKeyCustomField]
    }
    if (this.options.parentKeyCustomField && rawIssue.fields[this.options.parentKeyCustomField]) {
      issue.parentKey = rawIssue.fields[this.options.parentKeyCustomField]
    }
    return issue
  }
  ...
  search (jql, startAt = 0, maxResults = 50) {
    const fields = _.chain([
      ...
      'issuelinks',
      'updated',
      this.options.epicKeyCustomField,
      this.options.parentKeyCustomField
    ]).compact().join(',').value()
  ...
  async getIssuesByParentKey (keys, maxChunkSize = 10, pageSize = 1000) {
    const chunks = _.chunk(keys, maxChunkSize)
    const requests = chunks.map((chunk) => {
      return this.getIssuesByQuery(`"Parent Link" in (${chunk.join(',')})`, pageSize)
    })
    const issues = []
    for (let request of requests) {
      issues.push(...(await request))
