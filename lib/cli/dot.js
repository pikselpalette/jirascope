...
function dotifyGraph (graph) {
  ...
  stmts.push(...groupedGraphs['root'].nodes.map(dotifyIssue))
...
class Dot {
  constructor (config, jirascope) {
    ...
    this.rootDir = Path.resolve(this.config.path, 'output')
    this.dotDir = Path.resolve(this.rootDir, 'dot')
    this.graphsDir = Path.resolve(this.rootDir, 'graphs')
  }
  ...
  async run () {
    ...
    const writes = dots.map((dot, i) => {
      const dotFileName = Path.resolve(this.dotDir, `${i}.dot`)
    ...
    const execs = dots.map((dot, i) => {
      const dotFileName = Path.resolve(this.dotDir, `${i}.dot`)
      const pngFileName = Path.resolve(this.graphsDir, `${i}.png`)
