...
class Jirascope {
  ...
  async fetch () {
    ...
    _.forEach(issuesByKey, (issue) => {
      issue.links = _.filter(issue.links, (link) => {
        return issuesByKey[link.srcKey] && issuesByKey[link.dstKey] // only include edges that we have issues for
      })
    })

    // issues = _.orderBy(issues, helpers.bySortableKey)
    this.issues = _.orderBy(Object.values(issuesByKey), 'key')
  }
        ...
        while (stack.length !== 0) {
          ...
          if (issue) {
            ...
            issue.links.forEach((link) => {
              if (allIssuesByKey[link.srcKey] && allIssuesByKey[link.dstKey]) { // REMOVE-ME only include edges that we have issues for
...
const helpers = {
  sortableKey: (key) => { return [_.split(key, '-')[0], _.padStart(_.split(key, '-')[1], '0', 10)].join('-') },
