...
function score (graph) {
  ...
  const cumulativeSize = (key) => {
    ...
    node.analysis.cumulativeSize = _.sumBy(_.map(descendents[key], (key) => { return nodesByKey[key] }), 'analysis.size') + node.analysis.size
  }
  ...
  const unlockSize = (key) => {
    ...
    node.analysis.unlockSize = _.sumBy(_.map(ancestors[key], (key) => { return nodesByKey[key] }), 'analysis.size') + node.analysis.size
  }
