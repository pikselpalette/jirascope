...
function isAllowedProjectKey (issue, allowedProjectKeys) {
  ...
  return _.some(allowedProjectKeys, (allowedProjectKey) => { return _.startsWith(allowedProjectKey, issue.key) })
}
 ...
 * @property {Object} warnings - array of issue keys that have been flagged with warnings
 */
class Jirascope {
    ...
    while (newIssues.length > 0) {
      ...
      newIssues.forEach((newIssue) => {
        ...
        if (_.every(newIssue.links, {direction: 'outward'}) &&
          ...
          logger.trace(`${newIssue.key} - skipping`)
          return
        }
        ...
        if (newIssue.epicKey && !issues[newIssue.epicKey]) {
          missingIssues[newIssue.epicKey] = newIssue.epicKey
          logger.trace(`${newIssue.key} - ${newIssue.epicKey} epic not yet fetched`)
        }

        if (newIssue.type === 'Epic') {
          epicIssues[newIssue.key] = newIssue.key
          logger.trace(`${newIssue.key} - is an epic`)
        }
      })
      newIssues = []
      if (Object.keys(missingIssues).length > 0) {
        logger.info(`fetching linked issues`)
        newIssues = newIssues.concat(await this.jiraClient.getIssuesByKey(Object.keys(missingIssues)))
      }
      for (let epicIssueKey of Object.keys(epicIssues)) {
        const epicIssue = issues[epicIssueKey]
        logger.trace(`${epicIssueKey} - fetching epic child issues`)
  ...
  async store () {
    if (this.dataStore) {
      ...
      await this.dataStore.writeData('warnings', this.warnings)
    }
  }
  ...
  async cleanup () {
    ...
    if (this.dataStore) {
      ...
      await this.dataStore.deleteData('warnings')
    }
  }
}
