...
/**
 * Provides a lightweight data store using the local file system.
 *
...
class FileDataStore {
  ...
  async readData (resourceName) {
    const filename = Path.resolve(this.rootPath, 'data', `${resourceName}.json`)
    let data
  ...
  async deleteData (resourceName) {
    const filename = Path.resolve(this.rootPath, 'data', `${resourceName}.json`)
    await Fsp.remove(filename)
    logger.info(`deleted '${resourceName}.json'`)
  }
}

exports = module.exports = FileDataStore
