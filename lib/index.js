...
function determineGraphs () {
  const graphs = []
  const issues = _.chain(this.issues).reject('analysis.orphan').keyBy('key').value()
    ...
    if (stack.length !== 0) {
      ...
      while (stack.length !== 0) {
        ...
        if (issue) {
          ...
          node.analysis.main = this.options.allowedGraphIssueTypes.includes(node.type)
      ...
      if (_.filter(graph.nodes, 'analysis.main').length > 0) {
        graphs.push(graph)
      } else {
...
function determineSubgraphs () {
  ...
  this.graphs.forEach((parent) => {
    ...
    const mainNodes = _.filter(parent.nodes, 'analysis.main')
    mainNodes.forEach((main) => {
      const nodesByKey = _.keyBy(parent.nodes, 'key') // this wants to be fresh for each subgraph
      const stack = [main.key] // seed the stack
      const subgraph = {
        label: main.key,
        nodes: [],
        edges: []
      }
      logger.trace(`starting a new sub graph with ${main.key}`)
      while (stack.length !== 0) {
        const key = stack.pop()
        const node = _.cloneDeep(nodesByKey[key])
        logger.trace(`${key}: considering`)
        if (node) {
          subgraph.nodes.push(node)
          delete nodesByKey[key]

          node.analysis.main = node.key === main.key
          node.analysis.exit = !node.analysis.main && this.options.allowedGraphIssueTypes.includes(node.type)

          if (!node.analysis.leaf && !node.analysis.exit) {
            edgesBySrcKey[key].forEach((edge) => {
              stack.push(edge.dstKey)
              subgraph.edges.push(edge)
            })
          }
          if (!node.analysis.root && !node.analysis.exit) {
            edgesByDstKey[key].forEach((edge) => {
