...
function determineGraphs () {
  const graphs = []
  const issues = _.chain(this.issues).filter(['analysis.orphan', false]).keyBy('key').value()
  while (Object.keys(issues).length !== 0) {
    ...
    if (stack.length !== 0) {
      ...
      if (graph.nodes.length > 1) {
        graph.main = graph.nodes.filter((node) => { // find main nodes
          return this.options.allowedGraphIssueTypes.includes(node.type)
        })
        if (graph.main.length === 0) { // if no main nodes then bad graph
    ...
    parent.main.forEach((main) => {
      ...
      while (stack.length !== 0) {
        ...
        if (node) {
          ...
          if (node === main || !this.options.allowedGraphIssueTypes.includes(node.type)) {
            if (!node.analysis.leaf) {
              edgesBySrcKey[key].forEach((edge) => {
