...
class Repository {
  ...
  async issues () {
    const issueFileName = Path.join(this.config.path, 'issues.json')
    let issues
    if (this.config.readFromCache) {
      issues = await this.store.readJSON(issueFileName)
    }

    if (!issues) {
      issues = await fetchIssues(this.jiraClient, this.config.jql)
      if (this.config.writeToCache) {
        await this.store.writeJSON(issueFileName, issues)
      }
    }
