...
async function fetchIssues (jiraClient, jql) {
  ...
  Logger.global.info(`fetching initial issues`)
  ...
  while (newIssues.length > 0) {
    newIssues.forEach((newIssue) => {
      ...
      Logger.global.trace(`${newIssue.key} found`)
    })

    let missingIssues = {}
    Logger.global.info(`reviewing issue links`)
    newIssues.forEach((newIssue) => {
      if (newIssue.links.length === 0) {
        Logger.global.trace(`${newIssue.key} - no links`)
      } else if (newIssue.statusCategory === 'Done' && !this.config.followDone) {
        Logger.global.trace(`${newIssue.key} - skipping as 'Done'`)
      } else {
        Logger.global.trace(`${newIssue.key} - evaluating links`)
    ...
    if (lookups.length > 0) {
      Logger.global.info(`fetching linked issues`)
