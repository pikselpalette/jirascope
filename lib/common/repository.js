...
async function fetchIssues (jiraClient, jql) {
  ...
  let newIssues = await jiraClient.getIssues(jql)
  ...
  while (newIssues.length > 0) {
    ...
    newIssues.forEach((newIssue) => {
      newIssue.links.forEach((link) => {
        if (!issues[link.key]) {
          ...
          Logger.global.trace(`${newIssue.key} ${link.desc} ${link.key} - lookup required for ${link.key}`)
        } else {
    ...
    if (lookups.length > 0) {
      newIssues = await jiraClient.getIssues(`issue in (${lookups.join(',')})`)
    } else {
      newIssues = []
    }
  }
